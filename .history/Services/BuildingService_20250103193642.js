...
exports.createBuilding = expressAsyncHandler(async (req, res, next) => {
  const { levelss, kind, name, continued } = req.body;
  ...
  try {
    await building.save();
      ...
      if (continued === "first") {
        await createMainCategoryModel.findByIdAndUpdate(
          levels.id,
          {
            $pull: { building: building._id },
          },
          { new: true }
        );
      } else if (continued === "second") {
        await createCategoryModel.findByIdAndUpdate(levels.id, {
          $pull: { building: building._id },
        });
      } else if (continued === "third") {
        await createSubCategoryModel.findByIdAndUpdate(levels.id, {
          $pull: { building: building._id },
        });
      } else if (continued === "fourth") {
        await createNestSubCategoryModel.findByIdAndUpdate(levels.id, {
          $pull: { building: building._id },
        });
      } else if (continued === "fifth") {
        await createSubNestSubCategoryModel.findByIdAndUpdate(levels.id, {
          $pull: { building: building._id },
        });
      }
