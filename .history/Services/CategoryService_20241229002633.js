...
exports.getCategories = exports.getCategories = expressAsyncHandler(async (req, res) => {
  const categories = await createCategoryModel.find()
  .populate({
    path: "maincategories",
    select: "name", // إرجاع فقط الاسم من maincategories
  })
  .populate({
    path: "subcategories",
    select: "name", // إرجاع الاسم فقط من subcategories، بدون حقل categories
    populate: {
      path: "subcategories.categories",
      select: "-categories" // استبعاد حقل categories داخل كل subcategory
    }
  });
